...
class ChatFragment : Fragment() {
    ...
    private val stickerFolderLauncher = registerForActivityResult(
        ...
        treeUri?.let { uri ->
            ...
            if (stickers.isNotEmpty()) {
                Toast.makeText(requireContext(), getString(R.string.stealth_stickers_found, stickers.size), Toast.LENGTH_SHORT).show()
                showStickerTray(stickers)
            } else {
                Toast.makeText(requireContext(), R.string.stealth_no_stickers, Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun setupToolbar() {
        binding.toolbarRoomId.text = getString(R.string.stealth_room_default) + ": $roomId"
    ...
    private fun importWhatsAppChat(uri: Uri) {
        val progressDialog = AlertDialog.Builder(requireContext())
            .setTitle(R.string.stealth_importing)
            .setMessage(R.string.stealth_importing_progress)
            .setCancelable(false)
            .create()
        progressDialog.show()
                ...
                lifecycleScope.launch(Dispatchers.Main) {
                    if (progress.isComplete) {
                        ...
                        if (progress.error != null) {
                            Toast.makeText(requireContext(), getString(R.string.stealth_connection_failed, progress.error ?: ""), Toast.LENGTH_LONG).show()
                        } else {
                            Toast.makeText(
                                requireContext(),
                                getString(R.string.stealth_imported_messages, progress.importedMessages),
                                Toast.LENGTH_SHORT
                            ).show()
                            loadMessages()
                        }
    ...
    private fun showChangeAliasDialog() {
        ...
        AlertDialog.Builder(requireContext())
            .setTitle(R.string.stealth_change_alias)
            .setView(editText)
